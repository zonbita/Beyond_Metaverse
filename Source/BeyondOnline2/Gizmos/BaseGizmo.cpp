...
ETransformationDomain ABaseGizmo::GetTransformationDomain(USceneComponent* ComponentHit) const
{
	...
	//else
		//UE_LOG(Log, Warning, TEXT("Failed to Get Domain! Component Hit is not a Shape Component. %s"), *ComponentHit->GetName());
...
void ABaseGizmo::Call_Select_Highlight(UPrimitiveComponent* Component)
{
	Select_Highlight(Component);
}

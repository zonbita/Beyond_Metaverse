...
void ATransformerPawn::FilterHits(TArray<FHitResult>& outHits)
{
	...
	if (bIgnoreNonReplicatedObjects)
	{
		for (auto Iter = outHits.CreateIterator(); Iter; ++Iter)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Removing (Actor: %s   ComponentHit:  %s) from hits because it is not supported for networking.")
...
bool ATransformerPawn::MouseTraceByObjectTypes2(float TraceDistance, TArray<TEnumAsByte<ECollisionChannel>> CollisionChannels, TArray<AActor*> IgnoredActors, bool bAppendToList)
{
	FVector start, end;
	bool bTraceSuccessful = false;
	if (GetMouseStartEndPoints(TraceDistance, start, end))
...
bool ATransformerPawn::MouseTraceByChannel(float TraceDistance, TEnumAsByte<ECollisionChannel> TraceChannel, TArray<AActor*> IgnoredActors, bool bAppendToList)
{
	FVector start, end;
	bool bTraceSuccessful = false;
	if (GetMouseStartEndPoints(TraceDistance, start, end))
	{
		bTraceSuccessful = TraceByChannel(start, end, TraceChannel, IgnoredActors, bAppendToList);
		if (!bTraceSuccessful && !bAppendToList)
			ServerDeselectAll(false);
	}
	return false;
	...
	, bool bAppendToList)
{
	if (UWorld* world = GetWorld())
	{
		...
		if (world->LineTraceMultiByObjectType(OutHits, StartLocation, EndLocation, CollisionObjectQueryParams, CollisionQueryParams))
		{
			FilterHits(OutHits);
...
void ATransformerPawn::ApplyDeltaTransform(const FTransform& DeltaTransform)
{
	...
	for (auto& sc : SelectedComponents)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Transform will not affect Component [%s] as it is NOT Moveable!"), *sc->GetName());
		}
			
	}
}

void ATransformerPawn::ResetAllSelecter()
{
	ClearDomain();
}
...
void ATransformerPawn::SetTransformationType(ETransformationType TransformationType)
{
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("Setting Transformation Type to None!"));
    }
...
void ATransformerPawn::LogSelectedComponents()
{
	...
	UE_LOG(LogTemp, Log, TEXT("******************** SELECTED COMPONENTS LOG START ********************"));
    UE_LOG(LogTemp, Log, TEXT("   * Selected Component Count: %d"), SelectedComponents.Num());
	...
	for (int32 i = 0; i < SelectedComponents.Num(); ++i)
	{
        ...
        UE_LOG(LogTemp, Log, TEXT("   * [%d] %s"), i, *message);
	}

    UE_LOG(LogTemp, Log, TEXT("******************** SELECTED COMPONENTS LOG END   ********************"));
}
...
void ATransformerPawn::CheckUnreplicatedActors()
{
	...
	if (UnreplicatedComponentClones.Num() == 0)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("[SERVER] Time Elapsed for %d Replicated Actors to replicate: %f")

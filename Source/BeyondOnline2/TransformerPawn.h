...
UCLASS()
class BEYONDONLINE2_API ATransformerPawn : public APawn
{
	GENERATED_BODY()
...
public:
	...
	*/
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Runtime Transformer")
	bool MouseTraceByObjectTypes(float TraceDistance, TArray<TEnumAsByte<ECollisionChannel>> CollisionChannels, TArray<AActor*> IgnoredActors, bool bAppendToList = false);


	UFUNCTION(BlueprintCallable, Category = "Runtime Transformer")
		bool MouseTraceByObjectTypes2(float TraceDistance, TArray<TEnumAsByte<ECollisionChannel>> CollisionChannels, TArray<AActor*> IgnoredActors, bool bAppendToList = false);
	/**
	 * If a Gizmo is Present, (i.e. there is a Selected Object), then
	 * this test will prioritize finding a Gizmo, even if it is behind an object.
	 * If there is not a Gizmo present, the first Object encountered will be automatically Selected.
